...
/*  
      ...
      if(analogRead(IR.irArray[1]) <= 240){
        byte randNum = random(0,2);
          if(randNum >= 0.5){
            ...
            encoder0PosRight = 0;
            encoder0PosLeft = 0; 
            while (abs(encoder0PosRight) <= (2.15*rotDeg) and abs(encoder0PosLeft) <= (2.15*rotDeg)) {
              wheels.left_turn(255, 235);
            }
            wheels.stop_it();
            wheels.forward(255, 235);
  ...
  while (analogRead(IR.irArray[1]) > 1001) {

    wheels.right_turn(200, 0);
  }
  while (analogRead(IR.irArray[2]) > 1001) {
