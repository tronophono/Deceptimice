...
void enableInterrupt(uint8_t interruptDesignator, void (*userFunction)(void),
		uint8_t mode);
...
#include "../encoder/utility/ei_pindefs_speed.h"
#endif
...
const uint8_t PROGMEM digital_pin_to_port_bit_number_PGM[] = { 0, // 0 == port D, 0
		1, 2, 3, 4, 5, 6, 7, 0, // 8 == port B, 0
		1, 2, 3, 4, 5, 0, // 14 == port C, 0
		1, 2, 3, 4, 5 };
...
functionPointersPortD portDFunctions = { NULL, NULL, NULL, NULL, NULL, NULL,
		NULL, NULL };
...
void enableInterrupt(uint8_t interruptDesignator,
		interruptFunctionType userFunction, uint8_t mode) {
	uint8_t arduinoPin;
...
#include "../encoder/utility/ei_PinChange1284.h"
#elif defined ARDUINO_328
#include "../encoder/utility/ei_PinChange328.h"
#elif defined ARDUINO_MEGA
#include "../encoder/utility/ei_PinChange2560.h"
#elif defined ARDUINO_LEONARDO
#include "../encoder/utility/ei_PinChangeLeonardo.h"
#elif defined EI_ATTINY24
#include "../encoder/utility/ei_PinChangeTiny24.h"
#elif defined EI_ATTINY25
#include "../encoder/utility/ei_PinChangeTiny25.h"
#endif
...
#undef EI_SECTION_FALLING
		...
		portBitNumber = pgm_read_byte(
				&digital_pin_to_port_bit_number_PGM[arduinoPin]);
#endif
...
#undef EI_SECTION_ASSIGNFUNCTIONSREGISTERS
		...
		*pcmsk |= portMask; // appropriate PCMSKn bit is set, e.g. this could be PCMSK1 |= portMask;
		...
		origSREG = SREG;
		cli();
		// no interrupts while we're setting up an interrupt.

#define EI_SECTION_ENABLEEXTERNAL
#if defined MIGHTY1284
#include "../encoder/utility/ei_External1284.h"
#elif defined ARDUINO_328
#include "../encoder/utility/ei_External328.h"
#elif defined ARDUINO_MEGA
#include "../encoder/utility/ei_External2560.h"
#elif defined ARDUINO_LEONARDO
#include "../encoder/utility/ei_ExternalLeonardo.h"
#elif defined EI_ATTINY24
#include "../encoder/utility/ei_ExternalTiny24.h"
#elif defined EI_ATTINY25
#include "../encoder/utility/ei_ExternalTiny25.h"
#endif
...
#ifdef NEEDFORSPEED
#include "../encoder/utility/ei_porta_speed.h"
#else
...
#ifdef NEEDFORSPEED
#include "../encoder/utility/ei_portb_speed.h"
#else
	if (interruptMask == 0)
		goto exitPORTBISR;
	// get out quickly if not interested.
#ifdef EI_ARDUINO_INTERRUPTED_PIN
	...
	if (interruptMask & _BV(0))
		portBFunctions.pinZero();
	if (interruptMask & _BV(1))
		portBFunctions.pinOne();
	if (interruptMask & _BV(2))
		portBFunctions.pinTwo();
	if (interruptMask & _BV(3))
		portBFunctions.pinThree();
#if ! (defined EI_ATTINY24)
	if (interruptMask & _BV(4))
		portBFunctions.pinFour();
	if (interruptMask & _BV(5))
		portBFunctions.pinFive();
#endif
#if ! (defined ARDUINO_328) && ! (defined EI_ATTINY24) && ! (defined EI_ATTINY25)
...
#ifdef NEEDFORSPEED
#include "../encoder/utility/ei_portc_speed.h"
#else
	if (interruptMask == 0)
		goto exitPORTCISR;
	// get out quickly if not interested.
...
#else
	if (interruptMask & _BV(0))
		portCFunctions.pinZero();
	if (interruptMask & _BV(1))
		portCFunctions.pinOne();
	if (interruptMask & _BV(2))
		portCFunctions.pinTwo();
	if (interruptMask & _BV(3))
		portCFunctions.pinThree();
	if (interruptMask & _BV(4))
		portCFunctions.pinFour();
	if (interruptMask & _BV(5))
		portCFunctions.pinFive();
#ifdef MIGHTY1284
...
#ifdef NEEDFORSPEED
#include "../encoder/utility/ei_portd_speed.h"
#else
	if (interruptMask == 0)
		goto exitPORTDISR;
	// get out quickly if not interested.
...
#else
	if (interruptMask & _BV(0))
		portDFunctions.pinZero();
	if (interruptMask & _BV(1))
		portDFunctions.pinOne();
	if (interruptMask & _BV(2))
		portDFunctions.pinTwo();
	if (interruptMask & _BV(3))
		portDFunctions.pinThree();
	if (interruptMask & _BV(4))
		portDFunctions.pinFour();
	if (interruptMask & _BV(5))
		portDFunctions.pinFive();
	if (interruptMask & _BV(6))
		portDFunctions.pinSix();
	if (interruptMask & _BV(7))
		portDFunctions.pinSeven();
#endif // EI_ARDUINO_INTERRUPTED_PIN
	exitPORTDISR: return;
#endif // NEEDFORSPEED
}/*}}}*/
...
#ifdef NEEDFORSPEED
#include "../encoder/utility/ei_portj_speed.h"
#else
...
#ifdef NEEDFORSPEED
#include "../encoder/utility/ei_portk_speed.h"
#else
