...
void loop() {/*
  if (analogRead(IR.irArray[0]) < 1000) {
    if (analogRead(IR.irArray[1]) < 500) {
      byte randNum = random(0, 1);
      if (randNum >= 0.5) {
        delay(75);
        encoder0PosRight = 0;
        encoder0PosLeft = 0;
        while (abs(encoder0PosRight) <= (rotDeg)
            and abs(encoder0PosLeft) <= (rotDeg)) {
          wheels.left_turn(230, 230);
        }
        wheels.stop_it();
        wheels.forward(230, 230);
      } else {
        wheels.forward(230, 230);
        delay(400);
      }
    } else if (analogRead(IR.irArray[2]) < 500) {
      byte randNum = random(0, 1);
      if (randNum >= .5) {
        delay(75);
        encoder0PosRight = 0;
        encoder0PosLeft = 0;
        while (abs(encoder0PosRight) <= rotDeg and (encoder0PosLeft) <= rotDeg) {
          wheels.right_turn(230, 230);
  ...
  while (analogRead(IR.irArray[1]) > 1002) {

    wheels.right_turn(230, 230);
  }
  while (analogRead(IR.irArray[2]) > 1002) {
    wheels.left_turn(230, 230);
  }
*/
...
void checkingStates() {
  ...
  if (IR.state_left and IR.state_mid) {
    ...
    while (abs(encoder0PosRight) <= (2 * rotDeg)
        and abs(encoder0PosLeft) <= (2 * rotDeg)) {
      wheels.left_turn(230, 230);
    }
    wheels.stop_it();
    encoder0PosRight = 0;
