...
//Modify vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
//volatile const int fullRotationEncoder=42;//number of times
//encoder increments in one rotation.
volatile const int fullCellEncoder=640;//number of times encoder must //650 before
...
const int motorLa = 11;
const int motorLb = 6;
const int motorRa = 10;
const int motorRb = 8;
#define enablep1 7
#define enablep2 12
...
#define thresholdFront  600
#define thresholdLeft   800
#define thresholdRight  980
...
volatile unsigned long lastTime_encoder1; //this variable will be used to keep track of time using the function: millis(). millis() returns an unsigned long value that is the number of milliseconds that have passed since the device was powered on. The number of milliseconds that have passed can't be negative, hence the unsigned part and it's going to be a huge number, hence the long part. If the arduino is powered off, millis() will restart at 0. It can keep track of time for 50 days continuously, at which point it will reset back to 0.
  const int encoder_1APin =2; //this is the pin number for one of the encoder's outputs (A). The encoder has two outputs, which come from the two sensors under the rim of the tire.
  const int encoder_1BPin =3; //pin number for the encoder's other output (B).
volatile  int encoder_1Counter =0; //this variable will keep track of the number of times the wheel has rotated incrementally. There are 48 incremental rotations in one full rotation for the wheel. Basically, when this number reaches 48 the wheel made one full rotation. The number 48 comes from the fact that there are 12 teeth along the rim of the wheel. There are also 12 spaces in between the teeth. So, one sensor (A or B) can have 24 different readings in one rotation (I think HIGH corresponds to when a sensor is directly underneath one of the teeth and LOW corresponds to when the sensor is underneath one of the spaces). Because there are 2 sensors, 2*24=48. 
 ...
 volatile unsigned long lastTime_encoder2;
  const int encoder_2APin =4;
  const int encoder_2BPin =5;
...
{
/*
  Serial.print(encoder_2Counter);
  Serial.print(" ");
  Serial.println(encoder_1Counter);
  //Serial.print("\n");
  //turnRight();
*//*
  turnRight();
  ...
  uTurn();
 */
//  forward(240,230);//delay(5000);
//  backward(250);delay(5000);
//  getIR();
//  left(255,225);delay(1000);
//  stopp();delay(1000);
//  getIR();
...
//our code vvvvv
  ...
  digitalWrite(enablep1, HIGH);
  digitalWrite(enablep2, HIGH);

  pinMode(encoder_2APin, INPUT);
  pinMode(encoder_2BPin, INPUT);

  enableInterrupt(encoder_2APin, doEncoderLeftA, CHANGE);
  enableInterrupt(encoder_2BPin, doEncoderLeftB, CHANGE);

  pinMode(encoder_1APin, INPUT);
  pinMode(encoder_1BPin, INPUT);

  enableInterrupt(encoder_1APin, doEncoderRightA, CHANGE);
  enableInterrupt(encoder_1BPin, doEncoderRightB, CHANGE);
//our code ^^^^^^
}
...
{
    analogWrite(motorLa, 0); 
    analogWrite(motorRa, power);
    analogWrite(motorLb, powerR); 
    analogWrite(motorRb, 0);
}
...
/*
  ...
  Serial.println(sensorValueFour);
*/
}
...
void turnRight()
{
  int powerRightTurn=230;
  encoderStartCount=true;
      while(abs(encoder_1Counter) < rotations && encoder_2Counter < rotations)
    {
      right(power, powerRightTurn);
      if(abs(encoder_1Counter) < encoder_2Counter && powerRightTurn < 255)
          powerRightTurn = powerRightTurn + 5;
      else if (abs(encoder_1Counter) > encoder_2Counter && powerRightTurn > 190)
        powerRightTurn = powerRightTurn - 5;
...
void turnLeft()
{
  int powerLeftTurn=220;
  encoderStartCount=true;
   while(encoder_1Counter < (1.6*rotations) && abs(encoder_2Counter) < (1.6*rotations))
    {
      left(powerL,powerLeftTurn);
/*      if(encoder_1Counter < abs(encoder_2Counter) && powerLeftTurn < 255)
          powerLeftTurn = powerLeftTurn; + 5;
      else if (encoder_1Counter > abs(encoder_2Counter) && powerLeftTurn > 190)
        powerLeftTurn = powerLeftTurn; - 5;
*/
    }
    stopp();
...
//it's a double right turn
void uTurn()
{
  encoderStartCount=true;
      while(abs(encoder_1Counter) < (2.6*rotations) && abs(encoder_2Counter) < (2.6*rotations))
    {
      right(power, powerR);
      if(abs(encoder_1Counter) < abs(encoder_2Counter) && powerR < 255)
          powerR = powerR + 5;
      else if (abs(encoder_1Counter) > abs(encoder_2Counter) && powerR > 190)
        powerR = powerR - 5;
...
if(movementCalculated)
{
  ...
  encoder_2Counter=0;
  encoderStartCount=true;
  while( (encoder_1Counter<=fullCellEncoder) && (sensorValueTwo < 998) )
  {
    getIR();
   while(sensorValueOne > 996){//990
    getIR();
    right(0,200);
   }
   while(sensorValueSix > 1003){//997
    getIR();
    left(200,0);
   }

   if(encoder_1Counter< encoder_2Counter && powerR < 255)
        powerR = powerR + 5;
    else if (encoder_1Counter > encoder_2Counter && powerR > 190)
      powerR = powerR - 5;
...
void doEncoderRightA() {
  ...
  if (digitalRead(encoder_1BPin) == HIGH) {
    // check channel B to see which way encoder is turning
    if (digitalRead(encoder_1APin) == LOW) {
      encoder_1Counter = encoder_1Counter - 1;         // CW
    } else {
      encoder_1Counter = encoder_1Counter + 1;         // CCW
    }
  } else  // must be a high-to-low edge on channel A
  {
    // check channel B to see which way encoder is turning
    if (digitalRead(encoder_1APin) == HIGH) {
      encoder_1Counter = encoder_1Counter - 1;          // CW
    } else {
      encoder_1Counter = encoder_1Counter + 1;          // CCW
    }
...
void doEncoderRightB() {
  ...
  // Look for a high-to-low on channel B
    ...
    // check channel B to see which way encoder is turning
    if (digitalRead(encoder_1BPin) == LOW) {
      encoder_1Counter = encoder_1Counter - 1;          // CW
    } else {
      encoder_1Counter = encoder_1Counter + 1;          // CCW
    }
...
void doEncoderLeftA() {
  ...
  // look for a low-to-high on channel A
  if (digitalRead(encoder_2BPin) == HIGH) {
    // check channel B to see which way encoder is turning
    if (digitalRead(encoder_2APin) == LOW) {
      encoder_2Counter = encoder_2Counter + 1;         // CW
    } else {
      encoder_2Counter = encoder_2Counter - 1;         // CCW
    }
  } else  // must be a high-to-low edge on channel A
  {
    // check channel B to see which way encoder is turning
    if (digitalRead(encoder_2APin) == HIGH) {
      encoder_2Counter = encoder_2Counter + 1;          // CW
    } else {
      encoder_2Counter = encoder_2Counter - 1;          // CCW
    }
...
void doEncoderLeftB() {
  ...
  // Look for a high-to-low on channel B
    ...
    // check channel B to see which way encoder is turning
    if (digitalRead(encoder_2BPin) == LOW) {
      encoder_2Counter = encoder_2Counter + 1;          // CW
    } else {
      encoder_2Counter = encoder_2Counter - 1;          // CCW
    }
...
/* This is a custom function that I found from the arduino 
 * page: http://playground.arduino.cc/Main/PinChangeInterrupt
 * The webpage explains it better than I can. Using this
 * function will turn a digital pin into an interrupt pin.
 * Usually the digital pins can be inputs or outputs.
 * An interrupt pin is a special kind of input pin.
 * The arduino doesn't care whether an input pin changes
 * or not. The only time it will care about its value is
 * if digitalRead() is used. With an interrupt pin,
 * the arduino will become sensitive to every change in
 * the interrupt pin's state (LOW to HIGH or HIGH to LOW).
 * If the arduino detects a change of state in the interrupt
 * pin, the program will stop what it's doing and execute
 * the code in the ISR() function. When all that code is
 * finished executing, it will resume the main program
 * where it last left off. Later in the code, we're going
 * to use this function twice to turn encoderAPin and
 * encoderBPin into interrupt pins.
 */
/*
  {
  ...
  ISR(PCINT0_vect) 
  /* This is the Interrupt Service Routine. The value
   * inside the parenthisis corresponds to a range of pins.
   * If you look on an Arduino uno board you'll see 3 main
   * sections for input/output pins. The analog pins,
   * A0-A5 are in PCINT1_vect. The digital pins, D13-D8
   * are in PCINT0_vect. And the digital pins, D0-D7 are
   * in PCINT2_vect. When you're making the ISR() function
   * you have to use one of these 3 vectors. Let's say you
   * made digital pin 13 an interrupt pin, then you would
   * put PCINT0_vect in the parenthesis. When pin 13's state
   * changes, the code in your ISR() will execute immediately.
   * If for some reason you have 2 interrupt pins and
   * they're in different vectors, say digital pin 0 and
   * digital pin 13, then you have to make 2 ISR() functions,
   * one with PCINT0_vect in the parenthesis, and one
   * with PCINT2_vect in the other. If you have 2 interrupt
   * pins and they're in the same vector, say digital pin 13
   * and digital pin 12, (both in PCINT0_vect) then you only
   * make 1 ISR() function and a change in either pin will
   * trigger the code in ISR(PCINT0_vect)
   *
   * In our case, here, since encoderAPin (pin 8) and
   * encoderBPin (pin 11) are our interrupt pins and
   * they're in the same vector, we only need to make
   * 1 ISR() function and we use PCINT2_vect as our
...
/*
  {
    ...
    if(digitalRead(encoder_2APin)!=encoder_2ALastReading&&digitalRead(encoder_2BPin)==encoder_2BLastReading&&encoder2_aToggle)
/* There's 2 main parts to our ISR function (the code in 
 * the 2 if statements). The first if statement will only
 * execute if the current encoderAPin state doesn't match
 * the last reading (meaning encoderAPin's state just changed)
 * AND if encoderBPin's state is the same as the previous
 * reading (meaning encoderBPin's state didn't change) AND
 * if aToggle is true. To understand why aToggle is
 * neccessary, look at the truth table in this link(scroll
 * down just a bit)
 * http://www.allaboutcircuits.com/projects/how-to-use-a-rotary-encoder-in-a-mcu-based-project/
 * If you look at the truth table, you'll notice that at each
 * time transition (t1 to t2, t2 to t3, t3 to t4, etc.)
 * Only 1 output (A or B) is changing. They both can't
 * change at the same time (that's what the first 2
 * conditions of the if statement are for; to make sure
 * only 1 output is changing at a time.) Also notice how A
 * and B take turns alternating in changes. A changes,
 * then B changes, then A changes, etc. A can't change
 * twice in a row. It has to wait for B change before it
 * can change again. Same goes for B. That's what aToggle
 * and bToggle are for. Without these two toggles, one of
 * the outputs can be allowed to oscillate rapidly between
 * states without the other output changing at all (this
 * can happen when the tire is shaking or bouncing). If
 * this behavior occurs, it will give a false reading and
...
/*
    ...
    /* sets bToggle to true so that the next if statement
     * can execute.
     */
/*
      encoder2_bToggle=true; 
      }
